...
class v8DetectionLoss:
    """Criterion class for computing training losses for YOLOv8 object detection (single/dual head)."""

    def _filter_batch(self, batch, mask, cls_offset=0):
        # mask: (N,) bool
        out = {}
        for k, v in batch.items():
            if k in ("img", "im_file", "ori_shape", "resized_shape", "ratio_pad"):
                out[k] = v
        ...
        if "cls" in out and isinstance(out["cls"], torch.Tensor) and out["cls"].numel():
            out["cls"] = out["cls"].clone()
            out["cls"].view(-1).sub_(cls_offset)
        return out

    def __init__(self, model, tal_topk: int = 10):  # model must be de-paralleled
        device = next(model.parameters()).device
        h = model.args
        m = model.model[-1]  # Detect() or DualDetect()

        self.bce = nn.BCEWithLogitsLoss(reduction="none")
        self.hyp = h
        self.device = device

        # ------------------- detect whether dual-head -------------------
        self.is_dual = hasattr(m, "det_def") and hasattr(m, "det_roi")

        if self.is_dual:
            # two Detect heads
            self.det_def = m.det_def
            self.det_roi = m.det_roi

            # NOTE: assume both heads use same reg_max (normally 16)
            self.reg_max = int(self.det_def.reg_max)
            ...
            # self.stride_roi = self.det_roi.stride   # len=3 (P3-P5)

            self.defect_w = float(getattr(m, "defect_w", 1.0))

            # assigners + bbox losses (separate)
            self.assigner_def = TaskAlignedAssigner(topk=tal_topk, num_classes=self.nc_def, alpha=0.5, beta=6.0)
            self.assigner_roi = TaskAlignedAssigner(topk=tal_topk, num_classes=self.nc_roi, alpha=0.5, beta=6.0)

            self.bbox_loss_def = BboxLoss(self.reg_max).to(device)
            self.bbox_loss_roi = BboxLoss(self.reg_max).to(device)
    ...
    def _loss_single(self, feats, batch, stride, nc, assigner, bbox_loss):
        ...
        # ---------- stride 兜底：如果全 0，按层数自动给 ----------
        # (len=4 -> P2-P5: 4,8,16,32; len=3 -> P3-P5: 8,16,32)
        if stride is None:
            stride = torch.tensor([4, 8, 16, 32] if len(feats) == 4 else [8, 16, 32], device=self.device)
        elif isinstance(stride, torch.Tensor):
            if stride.numel() and float(stride.max()) == 0.0:
                stride = torch.tensor([4, 8, 16, 32] if len(feats) == 4 else [8, 16, 32], device=self.device)
            else:
        ...
        # ---------- concat feats ----------
        pred_distri, pred_scores = torch.cat([xi.view(bs, no, -1) for xi in feats], 2).split((reg_max * 4, nc), 1)
        pred_scores = pred_scores.permute(0, 2, 1).contiguous()  # (bs, N, nc)
        pred_distri = pred_distri.permute(0, 2, 1).contiguous()  # (bs, N, reg_max*4)

        dtype = pred_scores.dtype
        imgsz = torch.tensor(feats[0].shape[2:], device=self.device, dtype=dtype) * stride[0]
    ...
    def __call__(self, preds, batch):
        ...
        if getattr(self, "is_dual", False):
            ...
            if (
                ...
                # train path: (pred_roi_list, pred_def_list)
                pred_roi, pred_def = preds
            ...
            stride_roi = self.det_roi.stride
