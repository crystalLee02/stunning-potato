...
class DualDetect(nn.Module):
    ...
    def __init__(self, nc_roi=1, nc_def=2, defect_w=4.0, ch=()):
        super().__init__()
        self.nc_roi = int(nc_roi)
        self.nc_def = int(nc_def)
        self.nc_total = self.nc_roi + self.nc_def  # 3
        self.defect_w = float(defect_w)
    ...
    def bias_init(self):
        ...
        self.det_def.bias_init()
        self.det_roi.bias_init()

    @property
    def stride(self):
        return self.det_def.stride

    @stride.setter
    def stride(self, v):
        # v: tensor/list like [4,8,16,32]
        self.det_def.stride = v
        self.det_roi.stride = v[1:]  # [8,16,32]
    ...
    def forward(self, x):
        ...
        # --- unpack detect outputs ---
        if isinstance(p_def, (tuple, list)):
            y_def, x_def_raw = p_def
        else:
            y_def, x_def_raw = p_def, None

        if isinstance(p_roi, (tuple, list)):
            y_roi, x_roi_raw = p_roi
        else:
        ...
        print("y_def shape:", y_def.shape, "y_roi shape:", y_roi.shape)

        y_def3 = y_def.new_zeros((bs, 4 + self.nc_total, n_def))
        y_roi3 = y_roi.new_zeros((bs, 4 + self.nc_total, n_roi))
        ...
        # ROI -> global cls 0
        # y_roi: (bs, 5, n_roi) where cls at channel 4
        y_roi3[:, 4:5, :] = y_roi[:, 4:5, :]

        # DEF (0/1) -> global (1/2)
        # y_def: (bs, 6, n_def) where cls at channels 4:6
        y_def3[:, 5:7, :] = y_def[:, 4:6, :]

        # concat on anchor dimension (dim=2)
        y = torch.cat((y_roi3, y_def3), dim=2)

        aux = {"roi": x_roi_raw, "def": x_def_raw}
        return y if self.export else (y, aux)
