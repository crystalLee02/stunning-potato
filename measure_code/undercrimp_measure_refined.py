...
# 端子能量曲线可视化
# ==========================
def plot_energy_curve_cv(
    e_s,
    xBodyL=None, xBodyR=None,
    xL_sub=None, xR_sub=None,
    xL_step=None, xR_step=None,
    h=240
):
    """
    ...
    cv2.putText(canvas, f"Energy curve (n={n}) min={e_min:.1f} max={e_max:.1f}",
                (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (180, 180, 180), 2)
    cv2.putText(canvas, "Cyan: body | Green: xL/xR | Magenta: step",
                (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (180, 180, 180), 2)
...
def measure_undercrimp_terminal(
    ...
    txt = f"len={length_px:.2f}px ({used})" if length_mm is None else f"len={length_px:.2f}px={length_mm:.3f}mm ({used})"
    cv2.putText(vis, txt, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)

    dbg = dict(
        mask=mask, band_y=(y1, y2),
        xBodyL=xBodyL, xBodyR=xBodyR,
        thr=thr, used=used,
        xL_step=xL_step, xR_step=xR_step,
        xL_sub=xL_sub, xR_sub=xR_sub,
        snap_r=snap_r,
        e_s=e_s
    )
            ...
            if img is None:
                ...
                if under_conf < UNDER_THR:
                    ...
                    if crimp_xyxy is None:
                        cache[idx] = {"ok": False, "reason": "under_ok_but_no_class0", "img": img, "under_conf": under_conf}
                    else:
